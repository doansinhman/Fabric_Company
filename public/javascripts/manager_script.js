        ...
        function(data, status) {
            ...
            if (status == 'success' && data) {
                ...
                data.forEach(pro => {
                    PRODUCT[pro.id] = pro;
                    let tr = new TableRow();
                    tr.addTableData(pro.id);
                    tr.addTableData(pro.name);
                    tr.addTableData(pro.description);
                    tr.addTableData(pro.color);
                    tr.addTableData('<img style="width:20vw;" src="/images/product/' + pro.id + pro.image + '" alt="' + pro.name + '">');
                    tr.addTableData(pro.type);
                    tr.addTableData(pro.form);
                    tr.addTableData(pro.supplier);
                    tr.addTableData(pro.quantity);
                    tr.addTableData(pro.unit);
                    tr.addTableData(pro.unitPrice);
                    tr.addTableData('<button type="button" class="btn btn-outline-primary" onclick="editProduct(' + pro.id + ');">Sửa</button>\n\
                    <button type="button" class="btn btn-outline-danger" onclick="deleteProduct(' + pro.id + ');">Xoá</button>');
...
function editProduct(id) {
    pro = PRODUCT[id];
    document.updateProduct.id.value = pro.id;
    document.updateProduct.name.value = pro.name;
    document.updateProduct.description.value = pro.description;
    document.updateProduct.color.value = pro.color;
    document.updateProduct.type.value = pro.type;
    document.updateProduct.form.value = pro.form;
    document.updateProduct.supplier.value = pro.supplier;
    document.updateProduct.quantity.value = pro.quantity;
    document.updateProduct.unit.value = pro.unit;
    document.updateProduct.unitPrice.value = pro.unitPrice;
    openModal();
}

function deleteProduct(id) {
    if (confirm('Bạn có thực sự muốn xoá [' + id + ']: ' + PRODUCT[id].name + '?')) {
